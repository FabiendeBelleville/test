...
import presentation.ClientPres;
...
public class ImplementationService implements Service {
	...
	private ClientPres ccc = new ClientPres();

	@Override
	public void creerClient() {
		String nom = ccc.getNom();
		String prenom = ccc.getPrenom();
		String adresse = ccc.getAdresse();
		int cp = ccc.getCp();
		String ville = ccc.getVille();
		double firstDepotCC = ccc.getFirstDepotCC();
		double firstDepotCE = ccc.getFirstDepotCE();
	...
	public Client consulterClient(int id) {
		findClient ddd = new findClient();
		System.out.println(dao.findById(ddd.getId()));
		return dao.findById(ddd.getId());
	}
